...
const CategoryAttributes = ({ category, attributes, onChange }) => {
      ...
      getSize(category)
        .then((res) => {
          ...
          if (Array.isArray(sizeList) && sizeList.length > 0) {
            ...
            const normalized = sizeList.map((sz) => {
              if (typeof sz === 'string') {
                return { id: sz, name: sz };
              }
              // If already object, extract safe id/name
              return {
                id: sz.id || sz._id || sz.code || sz.name || sz.label || JSON.stringify(sz),
                name: sz.name || sz.label || sz.value || String(sz)
              };
            });
            setSizes(normalized);
          } else {
            // fallback to local attributeOptions
            const fallback = attributeOptions.sizes[category] || attributeOptions.sizes.default || [];
            const normalized = fallback.map((sz) => (
              typeof sz === 'string' ? { id: sz, name: sz } : { id: sz.id || sz._id || sz.name || sz.label, name: sz.name || sz.label || String(sz) }
            ));
            setSizes(normalized);
          }
        })
  ...
  const handleAddBrand = () => {
    ...
    if (
      brandCategories.allBrands.some(
  ...
  const colorMap = {
    ...
    Blue: "#0000FF",
    White: "#FFFFFF",
    Multi: "linear-gradient(45deg, red, blue, yellow)",
    Gray: "#808080",
    Pink: "#FFC0CB",
    Brown: "#A52A2A",
    Red: "#FF0000",
                ...
                {showAddBrand && (
                  <div className="p-3 border-b">
                    <input
                      ...
                      className={`w-full border rounded px-3 py-2 text-sm mb-3 focus:outline-none ${
                        brandError
                          ? "border-red-500"
      ...
      {visibleAttributes.includes("size") && (
        <div className="relative" ref={sizeRef}>
          ...
          {openSize && (
            <div className="absolute z-10 w-full bg-white shadow-lg rounded mt-2 max-h-96 overflow-y-auto">
              {sizes.map((s) => {
                const key = typeof s === 'string' ? s : (s.id || s._id || s.name || s.label);
