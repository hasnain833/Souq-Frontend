...
const ProfileDetails = ({ profileData, apiRefresh, seApiRefresh }) => {
  ...
  const [usernameChangeRemainingDays, setUsernameChangeRemainingDays] =
    useState(0);
      ...
      if (profileData.cityId) {
        ...
        try {
          ...
          if (citiesResponse.success) {
            console.log(
              "Available cities:",
              citiesResponse.data.cities.map((c) => c.name)
            );
            const matchingCity = citiesResponse.data.cities.find(
              (city) =>
                city.name.toLowerCase() === profileData.city.toLowerCase()
            );
            if (matchingCity) {
  ...
  const onSubmit = async (formData) => {
    ...
    const formattedBirthday = dayjs(profileData.dateOfBirth).format(
      "DD/MM/YYYY"
    );
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="bg-white shadow rounded-lg">
        ...
        <div className="border-t p-4 flex flex-col md:flex-row md:justify-between md:items-start gap-4">
          ...
          <input
            ...
            className={`border border-gray-300 rounded p-3 w-full md:w-1/3 focus:outline-none ${
              isUsernameDisabled
      ...
      <div className="pt-4 flex justify-end">
        <button
          ...
          className={`px-4 py-2 rounded-md transition-colors flex items-center justify-center text-white ${
            isSubmitting
              ? "bg-teal-600 cursor-not-allowed opacity-50"
              : "bg-teal-600 hover:bg-teal-700"
          }`}>
          {isSubmitting ? <LoadingSpinner /> : t("update_profile")}
        </button>
      </div>
    </form>
  );
