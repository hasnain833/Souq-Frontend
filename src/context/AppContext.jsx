...
export const AppProvider = ({ children }) => {
  ...
  const loadNotifications = async (page = 1, limit = 20, status = null, reset = true) => {
    try {
      setNotificationsLoading(true);
      const response = await NotificationService.getNotifications(page, limit, status);
  ...
  const deleteMultipleNotifications = async (notificationIds) => {
    try {
      ...
      // Update local state
      setNotifications(prev => {
        const deletedNotifications = prev.filter(n => notificationIds.includes(n._id));
        const unreadDeleted = deletedNotifications.filter(n => n.status === 'unread').length;
