    ...
    try {
      ...
      if (response.success) {
        ...
        if (!chatData || typeof chatData !== "object") {
          ...
          console.error(
            "Tried paths: response.data.data.chat, response.data.chat, response.data"
          );
        ...
        const transformedChat = {
          ...
          otherUser:
            chatData.buyer?.id === currentUser?.id
              ? chatData.seller
              : chatData.buyer,
          ...
          product: chatData.product
            ? {
                ...chatData.product,
                photos:
                  chatData.product.product_photos || chatData.product.photos,
              }
            : null,
        };
        ...
        if (offerAmount && parseFloat(offerAmount) > 0) {
          try {
            ...
            if (offerResponse.success) {
              ...
              const offerKey = `offer_processed_${
                chatData.product?.id || "unknown"
              }_${offerAmount}`;
              ...
              if (
                ...
                offerResponse.message.includes("already an active offer")
              ) {
                toast.info("You already have an active offer for this item.");
              } else {
                toast.warning(
                  "Chat created but offer failed to send. You can try again in the chat."
                );
              }
            }
          } catch (offerError) {
      ...
      {(!isMobile || (isMobile && !selectedChat)) && (
        <div
          className={`${
            isMobile
              ? "w-full"
              : "w-1/3 ltr:border-r rtl:border-l border-gray-200"
          } bg-white`}>
          <ChatList
            chats={chats}
