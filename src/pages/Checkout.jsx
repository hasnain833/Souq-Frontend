...
export default function CheckoutPage() {
  ...
  const total =
    productPrice +
    (useEscrow ? platformFee : buyerProtectionFee) +
    shipping +
    tax;
  ...
  const loadShippingRates = async () => {
    try {
      ...
      if (
        ...
        deliveryResponse.data.deliveryOptions &&
        providersResponse.success &&
        ...
        console.log(
          `✅ Found ${deliveryOptions.length} delivery options:`,
          deliveryOptions
        );
        ...
        const transformedRates = deliveryOptions.map((option) => {
          ...
          const providerData =
            providerMap[option.shippingProvider._id] || option.shippingProvider;
          ...
          const convertToUSD = (amount) => {
            ...
            if (providerName === "zajel") {
              ...
              console.log(
                `💱 Zajel conversion: ${amount} AED → ${convertedAmount} USD`
              );
          ...
          if (
            ...
            option.serviceCode === "LOCAL_PICKUP"
          ) {
            serviceCost = 0; // Local pickup is free
            serviceName = "Local Pickup";
            estimatedDays = { min: 0, max: 1 };
          } else if (
            providerName === "local_dropoff" ||
            option.serviceCode === "LOCAL_DROPOFF"
          ) {
            serviceCost = 0; // Drop-off point is free
            serviceName = "Drop-off Point";
            estimatedDays = { min: 0, max: 1 };
          } else if (
            providerName === "local_delivery" ||
            option.serviceCode === "LOCAL_DELIVERY"
          ) {
            ...
            const supportedService =
              providerData?.supportedServices?.find(
            ...
            if (supportedService) {
              ...
              estimatedDays = supportedService.estimatedDays || {
                min: 1,
                max: 3,
              };
            }
            ...
            console.log(`💰 Calculated cost for ${serviceName}:`, {
              ...
              conversionApplied:
                option.shippingProvider?.name?.toLowerCase() === "zajel" ||
          ...
          return {
            ...
            provider: {
              ...
              displayName:
                ...
                ShippingService.formatProviderName(
        ...
        console.log(
          `✅ Transformed ${transformedRates.length} shipping rates:`,
          transformedRates
        );
        ...
        if (!deliveryResponse.success) {
          ...
          toast.info(
            "No delivery options configured. Please set up delivery options in settings."
          );
        }
      }
    } catch (error) {
          ...
          <div className="bg-white p-4 rounded shadow">
            <div className="flex gap-4">
              ...
              {/* <img
                ...
                alt={product.name}
                className="w-40 h-40 rounded object-cover border border-gray-100"
              /> */}
              <div>
            ...
            {shippingLoading ? (
                        ...
                        <div className="flex items-center gap-2">
                          {rate.provider?.name === "zajel" ? (
                            <div className="w-8 h-8 bg-blue-100 rounded flex items-center justify-center">
                              ...
                              />
                              <span className="text-blue-600 text-xs font-bold hidden">
                                Z
                              </span>
                            </div>
                          ) : (
                            <div
                              className={`w-8 h-8 rounded flex items-center justify-center ${
                                rate.provider?.name === "aramex"
                                  ? "bg-red-100"
                                  : rate.provider?.name === "fetchr"
                                  ? "bg-green-100"
                                  : rate.provider?.name === "dhl"
                                  ? "bg-yellow-100"
                                  : "bg-orange-100"
                              }`}>
                              <span
                          ...
                          <div className="flex-1">
                            <p className="text-sm font-medium text-gray-900">
                              {rate.provider?.displayName || rate.serviceName} -{" "}
                              {rate.serviceName}
                            </p>
                            <p className="text-xs text-gray-500 mt-1">
                              {rate.estimatedDays?.min ===
                              rate.estimatedDays?.max
                                ? `${rate.estimatedDays.min} business day${
                                    rate.estimatedDays.min > 1 ? "s" : ""
                                  }`
              ...
              <div className="border p-4 rounded text-center">
                ...
                <p className="text-gray-600 text-sm mb-2">
                  {t("no_delivery_options")}
                </p>
                <p className="text-xs text-gray-500">
                  {t("setDeliveryOptions")}
                  <Link
                    to="/settings/delivery"
                    className="text-teal-600 hover:text-teal-700 ml-1 rtl:mr-1 hover:underline">
                    {t("settings")}
                  </Link>
                </p>
              </div>
              ...
              <div
                ...
                onClick={() =>
                  setSelectedShipping({
                    ...
                    provider: {
                      name: "default",
                      displayName: t("standard_shipping"),
                    },
                    serviceName: t("default_service"),
                    estimatedDays: product.shipping_cost.estimatedDays || {
                      min: 3,
                      max: 7,
                    },
                    cost: {
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:space-x-4 space-y-2 sm:space-y-0 rtl:space-x-reverse rtl:space-y-reverse">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-500">
                      • {t("funds_held_until_delivery")} •{" "}
                      {t("dispute_resolution")}• 10% {t("platformFee")}
                    </div>
                  </div>
                      ...
                      <div
                        ...
                        <div className="flex justify-between items-center">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              <p className="text-sm font-medium">
                                {getCardBrandInfo(card.cardBrand).name} ••••{" "}
                                {card.lastFourDigits}
                              </p>
                              <p className="text-xs text-gray-500">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span
                              style={{
                                color: getCardBrandInfo(card.cardBrand).color,
                              }}>
                ...
                {bankAccountsLoading ? (
                  ...
                  <div className="border p-3 rounded flex justify-between items-center mt-2">
                    <div>
                      ...
                      <p className="text-gray-500 text-sm">
                        Add a bank account from settings
                      </p>
                    </div>
                    <div className="text-2xl">🏦</div>
                  </div>
          ...
          {useEscrow ? (
            <button
              ...
              onClick={() => {
                ...
                if (!product || !productId) {
                  console.error(
                    "Checkout - Cannot navigate to escrow: product or product ID is missing"
                  );
                ...
                const navigationState = {
                  ...
                  offerAmount,
                  selectedCard:
                    paymentMethodType === "card" ? selectedCard : null,
                  selectedBankAccount:
                    paymentMethodType === "bank" ? selectedBankAccount : null,
                  paymentMethodType: paymentMethodType,
                  shippingAddress: selectedAddress,
                  selectedShipping: product?.shipping_cost,
                };
                console.log(
                  "Checkout - Navigating to escrow with state:",
                  navigationState
                );
            ...
            <button
              ...
              onClick={() => {
                ...
                if (!product || !productId) {
                  console.error(
                    "Checkout - Cannot navigate to standard payment: product or product ID is missing"
                  );
                  console.error("Product object:", product);
                  toast.error(
                    "Product information is missing. Please refresh the page and try again."
                ...
                console.log(
                  "Checkout - Navigating to standard payment with state:",
                  navigationState
                );
                navigate("/continue-checkout", { state: navigationState });
              }}>
