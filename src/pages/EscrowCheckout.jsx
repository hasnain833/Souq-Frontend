...
const EscrowCheckout = () => {
    ...
    const handleCreateEscrowTransaction = async () => {
        ...
        try {
            ...
            const totalAmountToPay = gatewayFeePaidBy === 'buyer' ? baseAmountForProcessing + processingFee : baseAmountForProcessing;
    ...
    const handleCreateStandardPayment = async () => {
        ...
        try {
            ...
            const standardPaymentSummary = {
                ...
                salesTax: salesTax,
                processingFee: gatewayFeePaidBy === 'buyer' ? processingFee : 0,
                totalAmount: gatewayFeePaidBy === 'buyer' ? baseAmountForProcessing + processingFee : baseAmountForProcessing,
                currency: selectedCurrency,
                exchangeRate: currentExchangeRate
            };
            ...
            if (initResponse.success) {
                ...
                if (initResponse.data.clientSecret) {
                    ...
                    console.log('✅ Stripe client secret received, navigating to payment page');
                    console.log('Client secret:', initResponse.data.clientSecret?.substring(0, 20) + '...');
                    console.log('Publishable key:', initResponse.data.publishableKey?.substring(0, 20) + '...');
    ...
    const handleCreatePayPalCardStandard = async () => {
        try {
            if (!selectedGateway || selectedGateway.id !== 'paypal') return toast.error('Please choose PayPal');
            ...
            const paymentResponse = await createStandardPayment(paymentData);
            if (!paymentResponse.success) return toast.error(paymentResponse.error || 'Failed to create payment');
            const paymentId = paymentResponse.data.paymentId;
            ...
            if (!initResponse.success) return toast.error(initResponse.error || 'Failed to initialize payment');
            ...
            if (initResponse.data?.transactionId) {
                navigate('/paypal-card-payment', {
                    state: { orderId: initResponse.data.transactionId, transactionId: paymentId, paymentType: 'standard', currency: selectedCurrency, paymentUrl: initResponse.data?.paymentUrl || null }
                });
                //   window.location.href = initResponse.data.paymentUrl;
            } else if (initResponse.data?.paymentUrl) {
    ...
    const handleCreatePayPalCardEscrow = async () => {
        try {
            if (!selectedGateway || selectedGateway.id !== 'paypal') return toast.error('Please choose PayPal');
            if (!agreementAccepted) return toast.error('Please accept the escrow agreement');
            if (!shippingAddress) return toast.error('Please add a shipping address');
            const finalProductId = productId || product?._id || product?.id;
            if (!finalProductId) return toast.error('Product information is missing');
            ...
            if (!transactionResponse.success) return toast.error(transactionResponse.error || 'Failed to create escrow transaction');
            const escrowTransaction = transactionResponse?.data?.data?.escrowTransaction;
            console.log('Escrow transaction:', escrowTransaction);
            if (!escrowTransaction || !escrowTransaction._id) {
                console.error('Unexpected escrow create response:', transactionResponse.data);
                return toast.error('Failed to create escrow transaction ');
            }

            const initResponse = await initializeEscrowPayment(escrowTransaction._id, {
                returnUrl: `${window.location.origin}/escrow/payment-success?transaction=${escrowTransaction._id}`,
                cancelUrl: `${window.location.origin}/escrow/payment-cancelled?transaction=${escrowTransaction._id}`
            });
            if (!initResponse.success) return toast.error(initResponse.error || 'Failed to initialize payment');
            ...
            if (initResponse.data?.data?.transactionId) {
                navigate('/paypal-card-payment', {
                    state: { orderId: initResponse.data.data?.transactionId, transactionId: initResponse?.data?.data?.transactionId, paymentType: 'escrow', currency: selectedCurrency, paymentUrl: initResponse.data?.paymentUrl || null }
                });
            } else if (initResponse.data?.paymentUrl) {
                window.location.href = initResponse.data.paymentUrl;
            } else {
    ...
    const processingFee = selectedGateway ?
        (baseAmountForProcessing * selectedGateway.feePercentage / 100) + (selectedGateway.fixedFee || 0) : 0;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                                ...
                                <div className="flex-1">
                                    <h3 className="font-medium text-gray-900 text-base line-clamp-2">{product.title}</h3>
                                    <p className="text-sm text-gray-600 mt-1 line-clamp-2">{product.description}</p>

                                    <div className="flex flex-wrap items-center gap-2 mt-2 text-sm text-gray-500">
                                        <span>{t("brand")}: {product.brand}</span>
                                        <span>{t("size")}: {product.size}</span>
                                        <span>{t("condition")}: {product.condition}</span>
                                    </div>
                                </div>
                                ...
                                <div className="text-left sm:text-right">
                                    {currencyLoading ? (
                                        <div className="flex items-center sm:justify-end gap-2">
                                            ...
                                            <span className="text-sm text-gray-600">{t("convertingTo")}...</span>
                                        </div>
                                    ) : (
                                        <>
                                            ...
                                            {offerAmount && (
                                                <div className="text-sm text-green-600">{t("offerApplied")}</div>
                                            )}
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                            <h2 className="text-lg font-semibold text-gray-900 mb-6">{t("currency_pricing")}</h2>
                                    ...
                                    <div className="px-4 py-3 border border-gray-200 rounded-lg bg-gradient-to-r from-gray-50 to-blue-50">
                                        {currencyLoading ? (
                                            <div className="flex items-center space-x-2 rtl:space-x-reverse">
                                                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-teal-600"></div>
                                                <span className="text-sm text-gray-600">{t("convertingTo")} {selectedCurrency}...</span>
                                            </div>
                                        ) : (
                                            <>
                                                ...
                                                {selectedCurrency !== 'USD' && (
                                                    <div className="text-xs text-gray-500 bg-white px-2 py-1 rounded-md inline-block">
                                                        {t("original")}: USD {(finalPrice || 0).toFixed(2)} (Rate: 1 USD = {(exchangeRate || 1).toFixed(4)} {selectedCurrency})
                                                    </div>
                                                )}
                                            </>
                                        )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  />
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                            ...
                            {isEditingAddress ? (
                                <div className="space-y-4">
                                    ...
                                    {/* Apartment */}
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">{t("apartmentOptional")}</label>
                                        <input
                                            type="text"
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">{t("state_province")}</label>
                                            <input
                                                type="text"
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">{t("address_type")}</label>
                                            <select
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            {shippingAddress.phoneNumber && (
                                                <p className="text-sm text-gray-600 flex items-center">
                                                    <span className="font-medium mr-1">{t("phone_number")}:</span>
                                                    {shippingAddress.phoneNumber}
                                                </p>
                                            )}
                                        </div>
                        ...
                        {/* Gateway Fee Options */}
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                            <h3 className="text-lg font-semibold text-gray-900 mb-4">{t("payment_processing_fee")}</h3>
                        ...
                        {paymentType !== 'standard' && (
                            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
                                <h3 className="text-lg font-semibold text-gray-900 mb-4">{t("escrow_protection_agreement")}</h3>

                                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                                    <div className="flex items-start gap-3 sm:gap-4">
                                        <Shield className="w-5 h-5 text-blue-600 mt-0.5 flex-shrink-0" />
                                        <div className="text-sm text-blue-800">
                                            <h4 className="font-medium mb-2">{t("how_escrow_protection_works")}:</h4>
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sticky top-8">
                            <h3 className="text-lg font-semibold text-gray-900 mb-4">{t("payment_summary")}</h3>
                            ...
                            <div className="space-y-3 text-sm">
                                <div className="flex justify-between">
                                    ...
                                    {currencyLoading ? (
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <span className="text-xs text-gray-500">{t("convertingTo")}...</span>
                                        </div>
                                    ) : (
                                ...
                                <div className="flex justify-between">
                                    <span className="text-gray-600">
                                        {paymentType === 'standard' ? t('platformFee') : t('escrow_protection_fee')}
                                    </span>
                                    {currencyLoading ? (
                                        <div className="flex items-center space-x-2">
                                            <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-teal-600"></div>
                                ...
                                {selectedCurrency !== 'USD' && !currencyLoading && (
                                    <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded">
                                        <div className="font-medium mb-1">{t("exchangeRate")}: 1 USD = {(exchangeRate || 1).toFixed(4)} {selectedCurrency}</div>
                                        <div className="space-y-1">
                                            <div>{t("originalProductPrice")}: USD {(finalPrice || 0).toFixed(2)}</div>
                                            <div>{t("originalPlatformFee")}: USD {((finalPrice || 0) * (paymentType === 'standard' ? 0.05 : 0.1)).toFixed(2)}</div>
                                            <div>{t("originalShipping")}: USD {(product.shipping_cost || 0).toFixed(2)}</div>
                                ...
                                <div className="border-t border-gray-200 pt-3">
                                    <div className="flex justify-between">
                                        <span className="font-semibold text-gray-900">{t("total_to_pay")}:</span>
                                        {currencyLoading ? (
                                            <div className="flex items-center space-x-2">
                                                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-teal-600"></div>
                  ...
                  <div>hasProductId: {!!(productId || product?._id || product?.id)}</div>
                            ...
                            <div className="mt-6 space-y-3">
                                ...
                                {selectedGateway?.id === 'paypal' ? (
                                    <PayPalButtonStack
                                        loading={loading}
                                        disabled={loading || productLoading || !shippingAddress || !(productId || product?._id || product?.id) || (paymentType !== 'standard' && !agreementAccepted)}
                                        onPayPal={paymentType === 'standard' ? handleCreateStandardPayment : handleCreateEscrowTransaction}
                                        onPayLater={paymentType === 'standard' ? handleCreateStandardPayment : handleCreateEscrowTransaction}
                                        onCard={paymentType === 'standard' ? handleCreatePayPalCardStandard : handleCreatePayPalCardEscrow}
                                    />
                                ) : (
                                    ...
                                    <>
                                        {paymentType === 'standard' ? (
                                            <button
                                                ...
                                                disabled={loading || productLoading || !selectedGateway || !shippingAddress || !(productId || product?._id || product?.id)}
