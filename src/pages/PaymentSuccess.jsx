...
import { getStandardPayment, checkStandardPaymentStatus } from '../api/StandardPaymentService';
...
const PaymentSuccess = () => {
    ...
    const autoCompletePaymentStatus = async (transaction, paymentType) => {
        ...
        try {
            ...
            if (paymentType === 'escrow') {
                console.log('🔄 Auto-completing escrow payment status for:', transaction.id);
                ...
                if (needsCompletion) {
                    ...
                    const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/user/escrow/${transaction.id}/complete-payment`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (response.ok) {
                        ...
                        console.log('⚠️ Escrow completion API call failed:', response.status);
                    }
                } else {
                    console.log('✅ Escrow payment already in correct status:', transaction.status);
                }
            } else {
                ...
                if (needsCompletion) {
                    const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/user/payments/${transaction.id}/check-payment-status`, {
                        method: 'GET',
                        headers: {
                            'Authorization': `Bearer ${accessToken}`
                        }
    ...
    const updateOrderStatusAfterPayment = async (transactionData) => {
        try {
            ...
            if (transactionData.paymentType === 'escrow') {
                toast.success('✅ Payment successful! Funds are held securely until delivery.');
            } else {
            ...
            toast.warning('Payment successful, but order status update failed. Please check your orders page.');
    ...
    useEffect(() => {
        if (transaction) {
            ...
            axios.put(`${API_BASE_URL}/api/user/product/${transaction.product?.id}/status`,
                { status: 'sold' },
                { headers: { Authorization: `Bearer ${token}` } }
            )
                .catch(err => {
                    // Handle error silently or with a toast if critical
                });
        }
    }, [transaction]);
            ...
            if (response && response.success) {
                ...
                if (paymentType === 'standard') {
                    ...
                    transformedTransaction = {
                        ...
                        totalAmount: paymentData.paymentSummary?.totalAmount || paymentData.totalAmount || paymentData.amount,
                    ...
                    <div className="p-6">
                        <div className="space-y-6">
                            ...
                            <div className="border-b border-gray-200 pb-6">
                                <h2 className="text-xl font-semibold text-gray-900 mb-4">{t("transactionDetails")}</h2>
                                    ...
                                    <div>
                                        ...
                                        <p className="font-medium text-gray-900">
                                            {transaction?.currency || 'USD'} {(
                                                ...
                                                transaction?.paymentSummary?.totalAmount + transaction.fees.platformFee ||
                                                transaction?.totalAmount + transaction.fees.platformFee ||
                                                transaction?.amount + transaction.fees.platformFee
                                            )?.toFixed(2) || 'N/A'}
                                        </p>
                                    </div>
                                    <div>
                                        ...
                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadgeClass(transaction?.status || 'completed')}`}>
                                            {(() => {
                                                ...
                                                if (paymentMethod === 'escrow') {
                                                    if (status === 'funds_held' || status === 'payment_processing' || status === 'completed') {
                                                        return 'funds held';
                                                    }
                                                }
                                                ...
                                                // Fallback logic
                                                if (status === 'payment_processing') return paymentMethod === 'escrow' ? 'funds held' : 'paid';
                                                if (status === 'processing') return paymentMethod === 'escrow' ? 'funds held' : 'paid';
                                                if (status === 'funds_held') return 'funds held';
                                                if (status === 'paid') return 'paid';
                                                if (status === 'completed') return paymentMethod === 'escrow' ? 'funds held' : 'paid';
                                                return paymentMethod === 'escrow' ? 'funds held' : 'paid';
                                            })()}
                                        </span>
                                    </div>
                                    {transaction?.fees && (
                                        <>
                                            <div>
                                                <p className="text-sm text-gray-500">{t("platformFee")}</p>
                                                <p className="font-medium text-gray-900">
                                                    {transaction.currency} {transaction.fees.platformFee?.toFixed(2) || '0.00'}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="text-sm text-gray-500">{t("gatewayFee")}</p>
                                                <p className="font-medium text-gray-900">
                            ...
                            {/* Product Info */}
                            <div className="border-b border-gray-200 pb-6">
                                <h3 className="text-lg font-semibold text-gray-900 mb-4">{t("productDetails")}</h3>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        {transaction?.escrowTransaction?.offer && (
                                            <div className="text-sm text-green-600 mb-2">
                                                <span className="font-medium">✅ {t("offer_accepted")}:</span> {transaction.currency || 'USD'} {transaction.escrowTransaction.offer.offerAmount?.toFixed(2)}
                                        ...
                                        {transaction?.product?.brand && (
                                            <p className="text-sm text-gray-500">
                                                {t("brand")}: <span className="text-gray-700">{transaction.product.brand}</span>
                                            </p>
                                        )}
                                        {transaction?.product?.condition && (
                                            <p className="text-sm text-gray-500">
                                                {t("condition")}: <span className="text-gray-700 capitalize">{transaction.product.condition}</span>
                                            </p>
                                        )}
                                    </div>
                                </div>
                            ...
                            {(transaction?.paymentType === 'escrow' || paymentType === 'escrow') ? (
                                <div className="bg-teal-50 rounded-lg p-4">
                                    <div className="flex items-start space-x-3 rtl:space-x-reverse">
                                        ...
                                        <div>
                                            <h3 className="font-medium text-teal-900">{t("escrowProtectionActive")}</h3>
                                    ...
                                    <div className="flex items-start space-x-3 rtl:space-x-reverse">
                                        ...
                                        <div>
                                            <h3 className="font-medium text-blue-900">
                                                {(transaction?.status === 'payment_processing' || !transaction) ? t('paymentProcessing') : t('standardPaymentCompleted')}
                                            </h3>
                            ...
                            <div className="bg-gray-50 rounded-lg p-4">
                                <h3 className="font-medium text-gray-900 mb-3">{t("whatHappensNext")}?</h3>
                                <div className="space-y-2">
                                    <div className="flex items-center space-x-3 rtl:space-x-reverse">
                                        ...
                                        <p className="text-sm text-gray-700">{t("sellerPreparesAndShips")}</p>
                                    </div>
                                    <div className="flex items-center space-x-3 rtl:space-x-reverse">
                                        <div className="w-6 h-6 bg-gray-300 text-gray-600 rounded-full flex items-center justify-center text-xs font-bold">
                                            2
                                        </div>
                                        <p className="text-sm text-gray-700">{t("youReceiveTracking")}</p>
                                    </div>
                                    {(transaction?.paymentType === 'escrow' || paymentType === 'escrow') && (
                                        <div className="flex items-center space-x-3 rtl:space-x-reverse">
                                            <div className="w-6 h-6 bg-gray-300 text-gray-600 rounded-full flex items-center justify-center text-xs font-bold">
                                            ...
                                            <p className="text-sm text-gray-700">{t("confirmDeliveryReleasePayment")}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            {transaction && (transaction.trackingId || transaction.tracking_id) && (
                                <div className="mt-6">
                                    <BuyerOrderTracking
                                        order={{
                                            id: transaction.id || transactionId,
                                            trackingId: transaction.trackingId || transaction.tracking_id,
                                            shippingProvider: transaction.shippingProvider || transaction.shipping_provider,
                                            status: transaction.status === 'completed' ? 'shipped' : transaction.status,
                                            estimatedDelivery: transaction.estimatedDelivery || transaction.estimated_delivery
                                        }}
                                    />
                                </div>
                            )}
              ...
              <div>
                ...
                >
                  ...
                  Check Payment Status
                        ...
                        {/* View Transaction Button - For both escrow and standard payments */}
                        {transactionId && (
                            <div>
                                <button
